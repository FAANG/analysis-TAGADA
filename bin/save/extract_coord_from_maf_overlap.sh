...
# Shell script
###############
# ~sdjebali/bin/extract_coord_from_maf_overlap.sh
...
EXTRACTCOORD=~sdjebali/Awk/extract_coord_from_maf_general.awk 
...
then
    ...
    echo "" >&2
    echo "Takes a gff or gtf file of segments and retrieves the multiple alignments they correspond to, provided that for a segment there is at least one multiple alignment (MA) that overlaps it." >&2
    echo "" >&2
    exit 0
fi
...
cat chr_names.txt | while read c; do $OVERLAP $base\_$c\_nr.gff $dirmareg/$c\_human_regions_hg19_with_MA_46species.gff -m -1 -f ma -o $base\_$c\_nr_withMAs_overlappingthem.gff; done
...
cat chr_names.txt | while read c; do awk -v refspecies=$assembly.$c -v searched_str=+ -v fileRef=$base\_$c\_nr_withMAs_overlappingthem_onlymareg_includingthem.bed -f $MAKEMULTIPLEMAF $MAFDIR/$c.maf; done
