...
workflow {
  ...
  // [prefix, [fastqs]] => [prefix, [fastqs]] & [reports]
  ...
  if (!params.index) {
    ...
    // genome & annotation & [fastqs] => index
    STAR_index_genome(
      channel_genome,
      channel_reference_annotation,
  ...
  // [prefix, [fastqs], index] => [prefix, bam] & report
  STAR_align_reads(
    channel_trimmed_reads.map({ trimmed ->
  ...
  // [prefix, bam, annotation] => [prefix, bam, length, direction]
  TAGADA_estimate_reads(
  ...
  if (!params.skip_assembly) {
    ...
    // [id, bam, direction, annotation] => assembly
    STRINGTIE_assemble_transcripts(
    ...
    // [assemblies] & annotation => annotation
    STRINGTIE_merge_assemblies(
  ...
  // [id, bam, length, direction, annotation, type] => [quantifications]
  STRINGTIE_quantify_expression(
  ...
  // genes_tpm_quantification & genes_counts_quantification => [reports]
  TAGADA_control_expression(
  ...
  // [reports] & config & custom_images & metadata
  MULTIQC_generate_report(
